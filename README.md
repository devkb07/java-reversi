...
The view is managed by [GUIView.java](src/reversi/GUIView.java), which visually builds the board using the Swing GUI library, creating the buttons constructed by [BoardSquareButton.java](src/reversi/BoardSquareButton.java). The controller is implemented with [ReversiController.java](src/reversi/ReversiController.java), responsible for handling the logic of the game, such as whether a given move is valid in the current state of the game.
